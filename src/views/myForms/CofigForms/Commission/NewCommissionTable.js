...
export default function NewCommissionTable({ newCommissionList, updateConfig, updateCommissionValue, deleteConfig, uploadNewCommissionConfig, createNewCommission }) {
  ...
  return (
    <Fragment>
      {newCommissionList?.length === 0 ? (
        <Alert color="danger">
          <h4 className="alert-heading">New Commission List is Empty</h4>
        </Alert>
      ) : (
        <Fragment>
          <Table size="sm" className="text-left border border-gray rounded-sm">
            ...
            <tbody>
              {newCommissionList?.map((item, idx) => {
                return (
                  <tr>
                    <td>{idx + 1}</td>
                    <td>
                    ...
                    <td>
                      ...
                      <EditCommission isOpen={editCommission} onClose={handleModal} updateCommissionValue={updateCommissionValue} item={item} />
                    </td>
                  </tr>
                )
              })}
          ...
          <div className="mt-1">
            <Button.Ripple color="primary" outline size="sm" onClick={() => {
              // uploadNewCommissionConfig
              createNewCommission(newCommissionList)
              }}>
              Upload Commission Configuration
            </Button.Ripple>
          </div>
        </Fragment>
      )}
