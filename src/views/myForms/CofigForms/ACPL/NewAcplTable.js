...
export default function NewAcplTable({ newAcplList, updateConfig, updateAcplValue, deleteConfig, uploadNewAcplConfig, createNewACPL }) {
  const { rto, insurer, policyType, oem, bodyType, fuelType, models, variants } = useSelector((store) => store.configReducer)
  const [editAcpl, setEditAcpl] = useState(false)
  const handleModal = () => {
    setEditAcpl(!editAcpl)
  }
  return (
    <Fragment>
      {newAcplList?.length === 0 ? (
        <Alert color="danger">
          <h4 className="alert-heading">New Acpl List is Empty</h4>
        </Alert>
      ) : (
        <Fragment>
              ...
              {newAcplList?.map((item, idx) => {
                return (
                  <tr>
                    ...
                    <EditAcpl isOpen={editAcpl} onClose={handleModal} updateAcplValue={updateAcplValue} item={item} />
                  </tr>
                )
              })}
            </tbody>
          ...
          <div className="mt-1">
            <Button.Ripple color="primary" outline size="sm" onClick={() => {
              // uploadNewAcplConfig
              createNewACPL(newAcplList)
            }}>
              Upload Acpl Configuration
            </Button.Ripple>
          </div>
        </Fragment>
      )}
